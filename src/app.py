...
from src.scheduler import start_task, pause_task, resume_task, cancel_task
...
def pick_folder() -> str | None:
    """Open native folder picker on macOS. Returns path or None."""
    if not IS_MACOS:
        return None
    try:
        result = subprocess.run(
            ["osascript", "-e",
             'POSIX path of (choose folder with prompt "选择视频存储路径")'],
...
with st.sidebar:
    ...
    platforms = st.multiselect(
        "平台", options=["YouTube", "X", "B站"], default=["YouTube"], key="platforms",
    )
    ...
    if duration_preset == "自定义":
        max_duration = st.number_input("自定义时长（分钟）", min_value=1, value=10, step=1, key="custom_dur")
...
for task in active_tasks:
    with st.container(border=True):
        ...
        ctrl1, ctrl2, ctrl3, _ = st.columns(4)
        with ctrl1:
